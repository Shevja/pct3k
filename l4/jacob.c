...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0)
    {
        ...
        if (rows < py)
        {
            fprintf(stderr, "Number of rows %d less then number of pyprocesses %d\n", rows, py);
            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }
        if (cols < px)
        {
            fprintf(stderr, "Number of cols %d less then number of pxprocesses %d\n", cols, px);
            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }
    ...
    // Allocate memory for local 2D subgridswith halo cells [0..ny + 1][0..nx + 1]
    ...
    if (ranky == 0)
    {
        ...
        for (int j = 1; j <= nx; j++)
        {
            // Translate col index to x coordin [0, 1]
            double x = dx * (sj + j - 1);
            int ind = IND(0, j);
    ...
    MPI_Type_commit(&col); // Top and bottom borders type
    MPI_Datatype row;
    ...
    for (;;)
    {
        ...
        // Swap grids (after termination local_gridwill contain result)
        double *p = local_grid;
    ...
    if (rank == 0)
    {
        ...
        printf("# procs %d : grid %d %d : niters %d : total time %.6f : mpitime %.6f : allred%.6f : halo %.6f\n", commsize, rows, cols, niters, prof[0], prof[1] + prof[2], prof[1], prof[2]);
